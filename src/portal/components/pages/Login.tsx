          ...
          <div className="rounded-2xl border border-navy-200/60 bg-white/80 backdrop-blur-xl shadow-xl shadow-navy-200/20 p-8">
            ...
            {screen === 'email' && (
              <>
                ...
                {/* ── Dummy / Testing Quick Accounts ── */}
                <div className="mt-6 pt-5 border-t border-navy-100">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-navy-400 text-center mb-3">
                    ⚡ Quick Testing Accounts (1-Click Login)
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      onClick={async () => {
                        setIsLoading(true);
                        const user = await login('sarah@globalcio.org', '');
                        if (user) navigate(resolveDestination(user.tier));
                        setIsLoading(false);
                      }}
                      className="flex items-center justify-between p-2.5 rounded-xl border border-blue-200 bg-blue-50/70 hover:bg-blue-100 text-left transition-all text-xs font-medium text-navy-800 shadow-sm"
                    >
                      <div>
                        <div className="font-bold text-blue-900">👑 Admin</div>
                        <div className="text-[10px] text-navy-500">Sarah Mitchell</div>
                      </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setIsLoading(true);
                        const user = await login('alex@globalcio.dev', '');
                        if (user) navigate(resolveDestination(user.tier));
                        setIsLoading(false);
                      }}
                      className="flex items-center justify-between p-2.5 rounded-xl border border-purple-200 bg-purple-50/70 hover:bg-purple-100 text-left transition-all text-xs font-medium text-navy-800 shadow-sm"
                    >
                      <div>
                        <div className="font-bold text-purple-900">💻 Dev Console</div>
                        <div className="text-[10px] text-navy-500">Alex Petrov</div>
                      </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setIsLoading(true);
                        const user = await login('robert@globalcio.org', '');
                        if (user) navigate(resolveDestination(user.tier));
                        setIsLoading(false);
                      }}
                      className="flex items-center justify-between p-2.5 rounded-xl border border-emerald-200 bg-emerald-50/70 hover:bg-emerald-100 text-left transition-all text-xs font-medium text-navy-800 shadow-sm"
                    >
                      <div>
                        <div className="font-bold text-emerald-900">🏢 CXO Member</div>
                        <div className="text-[10px] text-navy-500">Robert Chen</div>
                      </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        setIsLoading(true);
                        const user = await login('priya@nexarai.com', '');
                        if (user) navigate(resolveDestination(user.tier));
                        setIsLoading(false);
                      }}
                      className="flex items-center justify-between p-2.5 rounded-xl border border-amber-200 bg-amber-50/70 hover:bg-amber-100 text-left transition-all text-xs font-medium text-navy-800 shadow-sm"
                    >
                      <div>
                        <div className="font-bold text-amber-900">🚀 Startup</div>
                        <div className="text-[10px] text-navy-500">Priya Sharma</div>
                      </div>
